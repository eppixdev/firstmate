...
# Reuse fm-guard.sh's existing graced, owned-beacon check (FM_GUARD_GRACE) - do
# not duplicate the beacon math. Because the watcher rewrites its owned beacon
# every poll cycle, a normal fire leaves a recent matching beacon well inside
# grace and stays silent; only a genuine stale, missing, or mismatched lapse with
# work in flight warns. Call after the queue is emptied so guard never re-prints
# its own queued-wakes notice for the records this run just drained, and never
# let a guard hiccup change the drain's exit status.
