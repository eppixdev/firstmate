...
state/               volatile runtime signals; gitignored
  ...
  .last-watcher-beat watcher liveness beacon, atomically rewritten every poll with pid/path/home ownership metadata (including while absorbing benign wakes); fm-guard.sh reads it
...
- `NEEDS_GH_AUTH` - bootstrap already tried `gh auth status`, `GH_TOKEN` / `GITHUB_TOKEN`, and in a Codex session with `CODEX_SANDBOX_NETWORK_DISABLED=1` a local `gh` `hosts.yml` token.
  If the line still appears, ask the captain to run `! gh auth login` (interactive; you cannot run it for them).
...
Keepalive mode keeps the same durable wake queue and printed reason lines, starts the next watcher cycle immediately after an actionable wake so supervision is not down while waiting for a human prompt, then runs the normal `bin/fm-turn-sync.sh` path itself so queued wakes are consumed and the next actionable parked or blocked gate is surfaced even during a quiet stretch.
...
`bin/fm-watch-arm.sh` is self-verifying: it confirms a genuinely live watcher with a fresh beacon and prints exactly one honest status line - `watcher: started ...`, `watcher: healthy ...`, or `watcher: FAILED - no live watcher with a fresh beacon`.
...
In keepalive mode, actionable wakes still print and queue the same way, but the arm wrapper stays alive, immediately re-arms, and then drains through `bin/fm-turn-sync.sh`.
...
3. `stale:` the crewmate stopped without reporting; peek the pane (`bin/fm-peek.sh <window>`) to diagnose.
   If the pane is gone, immediately determine whether the task actually finished before telling the captain anything narrower about the missing pane: check for the scout report, a PR-ready or merged outcome, a local-only ready/merged outcome, or any other terminal deliverable, update backlog state, and tell the captain plainly whether the work is done and whether they can move on.
...
While running, `fm-watch.sh` rewrites `state/.last-watcher-beat` every poll cycle with the watcher's pid, pid-identity, watcher path, `FM_HOME`, and beat time.
The supervision scripts (`fm-peek`, `fm-send`, `fm-spawn`, `fm-teardown`, `fm-pr-check`, `fm-promote`, `fm-review-diff`, `fm-fleet-sync`, `fm-update`) call `bin/fm-guard.sh` first, which warns to stderr when any task is in flight (`state/*.meta` exists) but queued wakes are pending, or that beacon is missing, older than `FM_GUARD_GRACE` (default 300s), or does not belong to the live watcher recorded in this home's `state/.watch.lock`.
Equivalent path spellings for `FM_HOME` and `bin/fm-watch.sh` are normalized before persistence and comparison, so a symlinked or `/.`-spelled home does not look like a different watcher.
`bin/fm-wake-drain.sh` runs the same guard after it drains, so the liveness check also fires on a drain-and-handle turn that runs no other supervision script, narrowing the window in which a lapsed chain can hide; the owned grace beacon keeps it silent right after a normal fire and it warns only on a genuine stale-beyond-grace lapse.
...
When a worker disappears or exits unexpectedly, do not stop at reporting that the pane or window is gone.
First determine whether the work completed and tell the captain that outcome in the same update, including whether they can move on now.
