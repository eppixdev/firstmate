...
For GitHub auth specifically, bootstrap first trusts `gh auth status`.
If that fails, it still accepts `GH_TOKEN` or `GITHUB_TOKEN`.
In a Codex session with `CODEX_SANDBOX_NETWORK_DISABLED=1`, it also accepts a local `gh` login stored in `${GH_CONFIG_DIR:-${XDG_CONFIG_HOME:-$HOME/.config}/gh}/hosts.yml` so sandboxed `gh auth status` failures do not produce a false `NEEDS_GH_AUTH`.
Only when all of those checks fail does bootstrap print `NEEDS_GH_AUTH`.
...
FM_WATCH_WAKE_EXIT_STATUS=10   # exit code fm-watch-arm uses after printing a real watcher wake reason
FM_WATCH_ARM_KEEPALIVE_IDLE_SLEEP=5   # seconds --keepalive waits before rechecking a peer-owned healthy watcher
FM_WATCH_ARM_KEEPALIVE_RETRY_SLEEP=5  # seconds --keepalive waits before retrying after a failed arm child
FM_WATCHER_STALE_GRACE=300   # defaults to FM_GUARD_GRACE; seconds a live watcher lock may have a stale or mismatched owned beacon before re-arm errors
FM_WATCH_PRE_BEAT_DELAY=0   # test hook: delay before the watcher writes its first owned beacon
...
FM_AFK_START_CONFIRM_TIMEOUT=10    # seconds fm-afk-start waits for a tmux-launched supervise daemon to confirm its pid before rolling back .afk
