...
- The same universal requirements as tmux (a verified crew harness, git with GitHub auth via `gh auth login`, `GH_TOKEN`, or `GITHUB_TOKEN`, node, treehouse, no-mistakes, gh-axi, chrome-devtools-axi, and lavish-axi); treehouse still provides the worktree, zellij only provides the session.
...
Limitations: zellij is experimental, has no per-home workspace split (all tasks share one tab bar, unlike herdr), is not yet used for `bin/fm-bootstrap.sh`'s required-tools list (the version/tool gate happens at spawn time instead), has no verified agent-process liveness classifier for the session-start secondmate sweep, and carries the known gaps documented below (no native busy-state signal, and a narrow focus-steal race on tab creation) - see "Known gaps left for a follow-up" at the end of this document.
